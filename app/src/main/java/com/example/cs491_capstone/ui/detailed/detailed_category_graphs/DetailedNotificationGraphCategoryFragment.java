...
import static com.example.cs491_capstone.App.notificationColColor;
...
public class DetailedNotificationGraphCategoryFragment extends Fragment implements View.OnClickListener {
    ...
    private void generateColumnDataTopAsNotification() {
        ...
        for (int i = 0; i < numColumns; i++) {
            ...
            for (int j = 0; j < numSubcolumns; j++) {
                //
                int val = Integer.parseInt(App.localDatabase.getSumTotalStatByCategory(App.currentPeriod.get(0).get(i), DatabaseHelper.NOTIFICATIONS_COUNT, categoryName));

                if (val == 0) {
                    values.add(new SubcolumnValue(val, Color.TRANSPARENT));
                    break;
                } else {
                    values.add(new SubcolumnValue(val,  Color.parseColor(notificationColColor)));
                }
    ...
    private void generateColumnDataBottomAsNotifications(int columnIndex, int color) {
        ...
        for (int i = 0; i < barDataBottom.getColumns().size(); i++) {
            ...
            for (int j = 0; j < column.getValues().size(); j++) {
                ...
                int value = Integer.parseInt(App.localDatabase.getSumTotalStatByCategory(App.currentPeriod.get(0).get(columnIndex), String.valueOf(i), DatabaseHelper.NOTIFICATIONS_COUNT, categoryName));
